...
async def register_as_temporary(host, port, worker_id, original_master_address):
    ...
    Retorna True se a conexão e o envio ocorreram (o Master registra o worker
    como emprestado). Em seguida o Worker opera pelo ciclo da Sprint 02 enviando
    ALIVE com SERVER_UUID = Master de origem.
    """
    try:
        ...
        return False
    try:
        await messaging.send_message(writer, m2m.register_temporary_worker(
...
async def handle_command_redirect(msg, worker_id, current_origin=None):
    """Trata command_redirect: registra-se no novo Master e passa a operar lá.

    Retorna (host, port, origin_master) do novo Master, para o Worker entrar no
    work_loop com SERVER_UUID = Master de origem.
    """
...
async def handle_command_release(msg, worker_id):
    """Trata command_release: retorna ao Master de origem (apresentação Sprint 02)."""
